...
const updateCategories = (works) => {
  ...
  works.forEach((work) => {
    if (
      localCategories.findIndex((element) => element.id === work.categoryId) ==
      -1
    ) {
...
(async () => {
  let works = new Set(
    await fetch(`${API_URL}/works`).then((response) => response.json())
  );
  ...
  if (SessionManager().isAuthenticated()) {
    ...
    const categoriesLooped = async () => {
      let categories = await fetch(`${API_URL}/categories`).then((response) =>
        response.json()
      );
      ...
      works.forEach((work) => {
        worksLoop += `
                    <div class="work">
                        ...
                        <button class="work-edit">éditer</button>
                    </div>
                `;
      });
    ...
    addPicturebutton?.addEventListener("click", (e) => {
      e.preventDefault();
      document
        .querySelector(".modal-section")
        .parentNode.classList.add("slided");
    });
    ...
    buttonBack.addEventListener("click", (e) => {
      ...
      document
        .querySelector(".modal-section")
        .parentNode.classList.remove("slided");
    });
    document
    ...
    document
      .querySelector(".js-image-changer")
      .addEventListener("change", (e) => {
        ...
        if (AUTHORIZED_TYPE.includes(type)) {
          ...
          e.target.value = null;
          appendsFormError(
            `Le fichier doit être une image de type: jpeg,jpg,png`
          );
          return false;
        }
      });
...
const changeArrayForFilter = (works, filter) => {
  ...
  const filtered = [...localWorks].filter((element) =>
    filter.id !== 0 ? element.categoryId === filter.id : element
  );
